...
def doPack(bin_align, file_align, magic, dst_fn, binid_fn, ImgInfo_Offset, ap_c_fn, compress_format, DualImage_Offset):
    ...
    # datetime information (offset: 0x60)
    dt_bytes = struct.pack("BBBB",
                    (dt.hour),
                    (dt.day),
                    (dt.month),
                    (dt.year - 2000))
    f.seek((CHUNK_HEADER_BASE + 0x60))
    f.write(dt_bytes)

    # dual image rotating update image offset information (offset: 0x64)
    DualImage_Offset = struct.pack("BBBB",
                    (DualImage_Offset >> 0) & 0xFF,
                    (DualImage_Offset >> 8) & 0xFF,
                    (DualImage_Offset >> 16)  & 0xFF,
                    (DualImage_Offset >> 24)  & 0xFF)
    f.seek((CHUNK_HEADER_BASE + 0x64))
    f.write(DualImage_Offset)
...
if __name__ == '__main__':
    ...
    DualImage_Offset = int(args[12], 0)
    flash2_bin      = []
    ...
    if z[:6] == "CYGWIN":
		...
		MSCOMP7 = "./scripts/util/mscompress7 e 0 "
    doPack(bin_align, file_align, magic, dst_fn, binid_fn, ImgInfo_Offset, ap_c_fn, compress_format, DualImage_Offset)
